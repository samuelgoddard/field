    ...
    <div className="h-full flex flex-wrap py-8">
      ...
      <nav className="block w-full mt-auto pb-6 lg:pb-0">
        <ul>
          ...
          <li className="block px-8">
            <Link href="/about">
            <a className={`block text-xl leading-none py-2 ${route === '/about' && ''}`} onClick={() => modalEl.current.close()}>Terms &amp; Conditions</a>
            </Link>
          </li>
          <li className="block px-8">
            <Link href="/about">
            <a className={`block text-xl leading-none py-2 ${route === '/about' && ''}`} onClick={() => modalEl.current.close()}>Privacy Policy</a>
            </Link>
          </li>
        </ul>
      </nav>
